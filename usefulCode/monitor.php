...
function GetDisk()
{
    $d['t'] = round(@disk_total_space(".") / (1024 * 1024 * 1024), 3); //磁盘总量
    $d['f'] = round(@disk_free_space(".") / (1024 * 1024 * 1024), 3); //可用
    $d['u'] = $d['t'] - $d['f'];  //已用
    $d['PCT'] = (floatval($d['t']) != 0) ? round($d['u'] / $d['t'] * 100, 2) : 0; //使用率
    return $d;
}
...
if($a['PCT'] > 90) {
    ...
    $smtpusermail = "254500312@qq.com";//SMTP服务器的用户邮箱
    $smtpemailto = "xxx@163.com";//发送给谁
    $smtpuser = "254500312";//SMTP服务器的用户帐号
    $smtppass = "**********";//SMTP服务器的用户密码
    $mailsubject = "服务器满了！！！！！";//邮件主题
    $mailbody = "<h1>{$ip}服务器满了，磁盘使用率{$a['PCT']}%</h1>";//邮件内容
    $mailtype = "HTML";//邮件格式（HTML/TXT）,TXT为文本邮件##########################################
    $smtp = new Smpt($smtpserver,$smtpserverport,true, $smtpuser, $smtppass);//这里面的一个true是表示使用身份验证,否则不使用身份验证.
